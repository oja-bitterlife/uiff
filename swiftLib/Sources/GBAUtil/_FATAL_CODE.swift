...
public func initFatalFunc() {
    WorkMemory.setFatalFunc(fatalFunc: { code in
        dispCntMemory.writeUInt16(value: 0x0000)  // 画面OFF

        // 0x0300_0000は後で上書きするので一旦塗りつぶし色で
        workMemory.writeUInt16(value: RGB555(255, 0, 0))  // 赤色を表示するためのパレットデータを書き込む
        DMA3_UInt16(
        ...
        workMemory.writeUInt(value: UInt(bitPattern: code))

        // BG3CNTの設定(BG2の設定)
        dispCntMemory.writeUInt16(offset: 0x0c, value: 0)

        // BG2の表示ON
        let MD: UInt16 = UInt16(3)  // BitmapMode
        let BE: UInt16 = UInt16(1 << 2) << 8  // BG2 Enable
        dispCntMemory.writeUInt16(value: MD | BE)
    })
}
