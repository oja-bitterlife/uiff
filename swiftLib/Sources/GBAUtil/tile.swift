...
public struct BGTile {
    ...
    // タイル番号をタイルサイズ単位で取得する
    // --------------------------------------------------------------
    static public func getTileNo(_ tileX: Int, _ tileY: Int) -> Int {
...
public struct OBJTile {
    ...
    public func hide() {
        OAM.writeUInt16(offset: objNo * 8, value: 160)  // Y座標を画面外に設定して非表示
    }

    static public func clean() {
        for i in 0..<128 {
            OAM.writeUInt16(offset: i * 8, value: 160)  // Y座標を画面外に設定して非表示
        }
    }
}
